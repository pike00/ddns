...
parser = argparse.ArgumentParser(
    description="Use this command to update DNS entries in cloudflare"
)
...
if args.debug:
    ...
    logging.basicConfig(
        stream=sys.stderr, level=logging.ERROR, format="%(levelname)s: %(message)s"
    )
...
logging.debug(
    "Config File exists? (%s) %s", CONFIG_FILE, str(os.path.exists(CONFIG_FILE))
)
...
if not os.path.isfile(CONFIG_FILE):
    ...
    cloudflare_dict["domain"] = input(
        "Enter your Cloudflare Domain you'd like to update: "
    )

    dictionary = {"pushover": pushover_dict, "cloudflare": cloudflare_dict}
...
list_data = {"type": "A", "name": config["cloudflare"]["domain"]}
...
url_list = (
    ...
    + "/"
    + "dns_records?"
    + urllib.parse.urlencode(list_data)
)
...
# Create data to send
data = {"type": "A", "name": config["cloudflare"]["domain"], "content": ip, "ttl": 120}


cloudflare_req = requests.put(url_update, headers=headers, data=json.dumps(data))

dns_json = cloudflare_req.json()

if dns_json["success"]:
    logging.info(
        "DNS successfully updated. %s now points to %s",
    ...
    pushover_client.send_message(
        "DNS updated failed: " + json.dumps(dns_json["errors"])
    )
